...
| F-17 | pre-push steps · preflight steps · checkup checks | `grep -c '^step ' .githooks/pre-push ; grep -c 'step "' scripts/preflight.sh ; grep -c '^check "' scripts/checkup.sh` | 17 · 28 · 24 (was 10 — `e49aab6d`/#660 made the unit lanes opt-in behind `PREPUSH_TESTS=1`; +the Selling Guide corpus/coverage/watch lines, 2026-08-23. Preflight 29→28 and checkup 26→24 on 2026-09-15: the KB-index, doc-freshness and seat-roster lines were all the same script after #811, so the umbrella reported one guard as three passes) |
