...
describe("cron-jobs.yml schedules", () => {
  ...
  it("offers two CSRF-protected manual legs for the provider-backed extraction restart proof", () => {
    ...
    expect(workflow).toContain(
      'core-provider-canaries|core-storage-restart-seed|core-storage-restart-verify|core-extraction-restart-seed|core-extraction-restart-verify|core-tax-packet-canary|core-tax-packet-restart-seed|core-tax-packet-restart-verify) method="POST"',
    );
  ...
  it("offers two isolated exact-build legs for the 100-page restart proof", () => {
    for (const job of ["core-tax-packet-restart-seed", "core-tax-packet-restart-verify"]) {
      expect(jobs).toContain(`app.post("/api/jobs/${job}"`);
      expect(workflow).toContain(`- ${job}`);
    }
    const routes = jobs.slice(
      jobs.indexOf('app.post("/api/jobs/core-tax-packet-restart-seed"'),
      jobs.indexOf("const taxPacketCanaryError"),
    );
    expect(routes).toContain("kickCoreTaxPacketRestartWorker()");
    expect(routes).toContain("assertExpectedTaxRestartCommit(req)");
    expect(routes).not.toContain("kickDocumentExtractionWorker()");
  });
  ...
  it("prints only redacted provider diagnostics before failing a canary sweep", () => {
    ...
    expect(workflow).toContain('core-provider-canaries|core-storage-restart-seed|core-storage-restart-verify|core-extraction-restart-seed|core-extraction-restart-verify|core-tax-packet-canary|core-tax-packet-restart-seed|core-tax-packet-restart-verify)');
