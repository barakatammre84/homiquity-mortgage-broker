...
export function getCoreCapabilityReport(
  ...
  const configuredCapabilities: CapabilityConfig[] = [
    ...
    },
    {
      ...
      criticalForLiveLoan: true,
      lastSuccessfulVerificationAt: null,
      detail: "Deterministic rules, evidence gates, input and policy fingerprints, stale-output blocking, decision snapshots, and a manual-underwrite path are implemented; deployed repeatability is checked by synthetic canary.",
      nextAction: "Keep the deployed engine canary current, then run the selected live AUS path and retain its provider-native signed findings before lender handoff.",
    },
    {
      ...
      criticalForLiveLoan: true,
      lastSuccessfulVerificationAt: null,
      detail: "Self-employment, rental, tax tie-outs, workpapers, review checkpoints, and cited income packages are implemented; mixed-income repeatability is checked by synthetic canary.",
      nextAction: "Keep the deployed mixed-income canary current and keep uncited lender-specific methods blocked until their governing matrices are obtained.",
    },
    {
      id: "credit",
