...
export function HomiFinancialStory() {
  return (
    <div className="bg-precision-700 p-3 text-primary-foreground shadow-card-lg sm:p-5 lg:p-8" data-testid="homi-financial-story">
      <div className="border border-primary-foreground/15 bg-background p-4 text-foreground sm:p-6 lg:p-8">
        <div className="flex flex-wrap items-baseline justify-between gap-3 border-b border-precision-100 pb-5">
          <h2 className="font-serif text-2xl font-bold tracking-tight sm:text-3xl">Homi Financial Story</h2>
          <p className="text-xs font-semibold uppercase tracking-widest text-precision-500">Your information. One clear story.</p>
        </div>
        <div className="my-5 grid grid-cols-[1fr_auto] items-center gap-4 border border-precision-100 bg-muted px-4 py-4 sm:px-5" data-testid="homi-story-prompt">
          <p className="font-serif text-lg leading-snug sm:text-xl">I have a salary, consulting income, and two rentals.</p>
        ...
        <div className="relative grid gap-5 lg:grid-cols-[1.35fr_0.82fr] lg:gap-16">
          <div className="space-y-3">
            {INCOME_SOURCES.map((source) => (
              <div key={source.id} className="grid gap-2 border border-precision-100 bg-card px-4 py-3.5 sm:grid-cols-[1fr_auto] sm:items-center" data-testid={`homi-story-${source.id}`}>
                <span>
                  <span className="block font-serif text-base font-bold leading-tight sm:text-lg">{source.title}</span>
          ...
          <div className="flex items-center border border-precision-100 bg-precision-50 p-5 text-center lg:p-6">
            <div className="w-full">
              <p className="text-xs font-semibold uppercase tracking-widest text-precision-500">Connected file</p>
              <p className="mt-3 font-serif text-2xl font-bold leading-tight tracking-tight">One connected mortgage file</p>
              <p className="mt-3 text-sm leading-relaxed text-precision-500">Your income. Your properties. One complete story.</p>
              <Link href="/ai-coach" className="touch-target mt-5 inline-flex min-h-12 w-full items-center justify-center bg-flare-ink px-5 text-sm font-semibold text-primary-foreground transition-colors hover:bg-flare-ink/90" data-testid="homi-story-continue">
                Continue my plan
              </Link>
            </div>
          </div>
